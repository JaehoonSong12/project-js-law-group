...
\begin{itemize}
    \item \textbf{Landing State:} Users see the home page with the Auto Accident Wizard in the hero section, plus call-to-action, main-website link, and practice-area summary (FR-03 in \ref{sec:requirements}). The Floating Action Button (FAB) provides language selection (\texttt{en-US}, \texttt{ko-KR}, \texttt{ja-JP}, \texttt{es-US}) with persistence in LocalStorage (\ref{uc:01}).
