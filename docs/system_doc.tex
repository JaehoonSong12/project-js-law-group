...
use case diagram, and provides fully dressed use case descriptions for development and QA.\footnote{
  \textbf{Activity diagrams} and \textbf{state diagrams} have been omitted, 
...
\end{figure}
\newpage
\subsection{UI Flow Diagram}\label{ss:uiflow}
...
\begin{itemize}
    \item \textbf{Landing State:} Users see the home page with the Auto Accident Wizard in the hero section, plus call-to-action, main-website link, and practice-area summary (FR-03 in \ref{sec:requirements}). The Floating Action Button (FAB) provides language selection (en-US, ko-KR, ja-JP, es-US) with persistence in LocalStorage (\ref{uc:01}).
    \item \textbf{Wizard State:} A single, progressive 10-step form (accident type, ZIP, medical treatment, police report, prior attorney, accident date, injury type, name, contact details, submit). It is the sole form for case inquiries (FR-10 in \ref{sec:requirements}). Practice area pages link back to the wizard (e.g.\ \texttt{\#wizard-container}) for inquiry CTAs.
    \item \textbf{Practice Area State:} Dedicated routes for \texttt{/motor-vehicle-accident} and \texttt{/personal-injury}. The Personal Injury page includes a Criminal Defense section; there is no separate Criminal Defense route. Content is minimal and i18n-enabled (FR-08 in \ref{sec:requirements}).
    \item \textbf{Submission State:} After server-side validation (Flask-WTF, CSRF), the user sees a success modal; data is written to \texttt{submissions/} (JSON, CSV, HTML) and an email is sent via \texttt{GmailProxy}. Twilio follow-up and Mailchimp registration are as specified in \ref{uc:06} and \ref{uc:07}.
\end{itemize}
...
pattern adapted for Flask, deployed as a containerized application on cloud infrastructure.\footnote{
  \textbf{Class diagram} and \textbf{Object diagram} have been omitted, 
  as the system does not have the complexity that would warrant those diagrams.
}
